...
try {
  ...
  for (const width of [1440, 390]) {
    ...
    await expect(drawer.locator('.cart-line-total')).toHaveText('3 870 ₽')
    await drawer.locator('input[name=quantity]').fill('0')
    await expect(drawer.locator('[data-quantity-status]')).toContainText('допустимое целое количество')
    await expect(drawer.locator('.cart-total strong')).toHaveText('3 870 ₽')
    await drawer.locator('input[name=quantity]').fill('3')
    await expect(drawer.locator('[data-quantity-status]')).toBeHidden()
    ...
    await page.locator('.cart-page input[name=quantity]').fill('4')
    await expect(page.locator('.cart-page .cart-total strong')).toHaveText('5 160 ₽')
    await page.reload()
    await expect(page.locator('.cart-page .cart-total strong')).toHaveText('5 160 ₽')
