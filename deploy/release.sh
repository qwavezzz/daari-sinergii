...
systemd-run --quiet --wait --pipe --collect --uid=dari --gid=www-data --working-directory="$release_path" -p EnvironmentFile=/etc/dari/dari.env "$release_path/.venv/bin/python" manage.py check --deploy
if [ -L /srv/dari/current ]; then
    systemctl start dari-backup.service
    ln -sfn "$(readlink -f /srv/dari/current)" /srv/dari/previous
fi
systemd-run --quiet --wait --pipe --collect --uid=dari --gid=www-data --working-directory="$release_path" -p EnvironmentFile=/etc/dari/dari.env "$release_path/.venv/bin/python" manage.py migrate --noinput
systemd-run --quiet --wait --pipe --collect --uid=dari --gid=www-data --working-directory="$release_path" -p EnvironmentFile=/etc/dari/dari.env "$release_path/.venv/bin/python" manage.py setup_roles
systemd-run --quiet --wait --pipe --collect --uid=dari --gid=www-data --working-directory="$release_path" -p EnvironmentFile=/etc/dari/dari.env "$release_path/.venv/bin/python" manage.py collectstatic --noinput
